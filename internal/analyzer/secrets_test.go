...
func TestScanSecrets(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			dir := t.TempDir()
			_ = os.WriteFile(filepath.Join(dir, "test.go"), []byte(tc.content), 0o644)
...
func TestScanSecrets_SkipsBinary(t *testing.T) {
	dir := t.TempDir()
	_ = os.WriteFile(filepath.Join(dir, "image.png"), []byte("AKIAIOSFODNN7EXAMPLE"), 0o644)
	findings := ScanSecrets(dir)
	if len(findings) != 0 {
...
func TestScanSecrets_SkipsGitDir(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(dir, ".git", "config"), []byte("AKIAIOSFODNN7EXAMPLE"), 0o644)
	findings := ScanSecrets(dir)
	if len(findings) != 0 {
...
func TestScanSecrets_SkipsLargeFiles(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(dir, "big.txt"), large, 0o644)
	findings := ScanSecrets(dir)
	if len(findings) != 0 {
...
func TestScanSecrets_SkipsTestFiles(t *testing.T) {
	dir := t.TempDir()
	_ = os.WriteFile(filepath.Join(dir, "main_test.go"), []byte(`password = "supersecret123"`), 0o644)
	findings := ScanSecrets(dir)
	if len(findings) != 0 {
